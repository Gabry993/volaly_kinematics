...
class HeadFingerModel(PointingModel):
    ...
    def pointing_ray(self):
        ...
        try:
            origin_tf = self._tf_buff.lookup_transform(
                self._fixed_frame, self._ray_origin_frame, rclpy.time.Time()
            )
            pass_tf = self._tf_buff.lookup_transform(
                self._fixed_frame, self._ray_pass_frame, rclpy.time.Time()
            )
        except (
            ...
            rclpy.logging.get_logger("pointing_model").error(
                str(e)
            )  # TODO check logthrottle
            return None

        pos_origin = kdl.Vector(
        ...
        pos_pass = kdl.Vector(
            pass_tf.transform.translation.x,
